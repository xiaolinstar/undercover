...
**工厂模式**: `AppFactory` 类负责创建和配置 Flask 应用
**优势**: 解耦应用创建过程，便于测试和配置管理

**仓储模式**: `RoomRepository` 和 `UserRepository` 负责数据访问
**优势**: 抽象数据访问逻辑，便于更换存储方案

**服务模式**: `GameService` 和 `MessageService` 封装业务逻辑
**优势**: 分离关注点，提高代码复用性

**配置模式**: `GameConfig` 类集中管理游戏配置
**优势**: 统一配置管理，便于调整和维护
...
**模型层**: 房间实体，包含房间状态、玩家列表、游戏数据等
- **Room**: 房间实体，包含房间状态、玩家列表、游戏数据等
- **User**: 用户实体，包含用户信息、房间关联等

**服务层**: 核心游戏逻辑，包括房间管理、游戏控制、投票等
...
**插件化词语库**: 通过配置文件管理词语库，支持动态添加/删除词语对

**可配置游戏规则**: 房间超时时间可配置，玩家数量规则可调整，卧底分配策略可扩展

**模块化架构**: 各层之间松耦合，便于替换和升级组件，支持微服务拆分

## 性能优化

**Redis 过期策略**: 利用 Redis 原生过期机制自动清理超时房间，减少手动清理开销

**连接池管理**: Redis 连接复用，减少连接建立开销

**缓存策略**: 热点数据缓存，减少重复计算

## 安全设计

**微信签名验证**: 确保消息来源合法性，防止恶意请求

**数据隔离**: 用户数据相互隔离，房间数据独立管理

**输入验证**: 严格验证用户输入，防止注入攻击

## 测试策略

**单元测试**: 覆盖各层核心逻辑，使用 Mock 隔离外部依赖

**集成测试**: 验证组件协同工作，测试完整业务流程
...
**健康检查**: `/health` 接口监控应用状态，Redis 连接状态检查

**日志记录**: 关键操作日志记录，异常信息追踪，性能指标收集
