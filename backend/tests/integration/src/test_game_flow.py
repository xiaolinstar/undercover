...
class TestGameFlow:
    ...
    def test_full_game_flow(self, game_service):
        ...
        # 不再检查具体的房间ID，因为现在使用Snowflake算法生成
        ...
        # 3. 房主开始游戏
        with patch("backend.services.game_service.WordPair") as mock_word_pair:
            mock_obj = Mock()
            ...
            success, result = game_service.start_game(room_id, "user1")
        assert success is True
        ...
        # 6. 房主投票
        with (
            patch("backend.services.game_service.SQLUser") as mock_sql_user,
            patch("backend.services.game_service.GameRecord"),
            patch("backend.services.game_service.db"),
        ):
